// ==UserScript==
// @name         SnapMoe - 二次元图片收藏助手
// @version      2.6.6
// @description  SnapMoe - 基于 Cloudflare Worker + R2 的二次元图片收藏工具
// @match        *://*/*
...
(function () {
  ...
  function initConfig() {
    ...
    if (GM_getValue("WORKER_URL") === undefined) {
      setConfig(
        "WORKER_URL",
        "https://pic-collect-upload.YOUR-ACCOUNT.workers.dev/"
      );
    }
    if (GM_getValue("SUPABASE_URL") === undefined) {
      setConfig("SUPABASE_URL", "https://your-supabase-url.supabase.co");
    }
    if (GM_getValue("SUPABASE_ANON_KEY") === undefined) {
      setConfig("SUPABASE_ANON_KEY", "your-supabase-anon-key");
    }
    // API Key 是可选的，不设置默认值
  }
  ...
  const WORKER_API_KEY = getConfig("WORKER_API_KEY", ""); // 可选，生产环境建议配置
  ...
  function validateConfig() {
    ...
    if (WORKER_URL.includes("YOUR-ACCOUNT")) {
      warnings.push("⚠️ WORKER_URL 未配置，请在 Tampermonkey 脚本设置中配置");
    }
    ...
    if (!WORKER_API_KEY || WORKER_API_KEY === "") {
      warnings.push(
    ...
    if (warnings.length > 0) {
      ...
      console.log(
        "[SnapMoe] 💡 配置方法：在 Tampermonkey 管理面板中，找到此脚本，点击编辑，在脚本开头找到配置相关代码，使用 GM_setValue 设置配置值"
      );
      console.log("[SnapMoe] 💡 或在浏览器控制台执行：");
      console.log("[SnapMoe]    GM_setValue('WORKER_URL', '你的 Worker URL')");
      console.log(
        "[SnapMoe]    GM_setValue('SUPABASE_URL', '你的 Supabase URL')"
      );
      console.log(
        "[SnapMoe]    GM_setValue('SUPABASE_ANON_KEY', '你的 Supabase Anon Key')"
      );
      console.log(
        "[SnapMoe]    GM_setValue('WORKER_API_KEY', '你的 Worker API Key')"
      );
    }
  }
  ...
  // ========== 已收藏图片缓存 ==========
  let collectedImagesMap = new Map(); // 存储当前页面已收藏的图片：URL -> {file_hash, r2_path}
    ...
    try {
      ...
      const blob = await new Promise((resolve, reject) => {
        GM_xmlhttpRequest({
          ...
          onerror: (error) => {
            console.error("[SnapMoe] 网络错误:", error);
      ...
      try {
        ...
        console.warn("[SnapMoe] 无法获取图片尺寸:", e);
      }
